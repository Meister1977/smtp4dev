...
namespace Rnwood.Smtp4dev
{
    public partial class MainForm : Form
    {
        ...
        public MessageViewModel SelectedMessage
        {
            get
            {
                if (messageGrid.SelectedRows.Count != 1) return null;
        ...
        public SessionViewModel[] SelectedSessions
        {
            ...
            {
                return
                    sessionsGrid.SelectedRows.Cast<DataGridViewRow>()
                        .Select(row => (SessionViewModel) row.DataBoundItem)
                        .ToArray();
            }
        }
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            if (_firstShown)
            {
                ...
                if (Settings.Default.ListenOnStartup) StartServer();
            }
        ...
        private void StartServer()
        {
            ...
            trayIcon.Text = $"smtp4dev (listening on :{Settings.Default.PortNumber})\n{_messages.Count} messages";
        ...
        private void OnMessageReceived(object sender, MessageEventArgs e)
        {
            ...
            Invoke((MethodInvoker) (() =>
            {
                ...
                {
                    if (Settings.Default.AutoViewNewMessages) ViewMessage(message);

                    if (Settings.Default.AutoInspectNewMessages) InspectMessage(message);
                }
        ...
        {
            foreach (var message in SelectedMessages) ViewMessage(message);
        }
        ...
        private void StopServer()
        {
            ...
            trayIcon.Text = $"smtp4dev (not listening)\n{_messages.Count} messages";
        }
        ...
        {
            foreach (var message in SelectedMessages) _messages.Remove(message);

            foreach (var session in _sessions
                .Where(s => !_messages.Any(mvm => s.Session.Messages.Contains(mvm.Message)))
                .ToArray()) _sessions.Remove(session);
        }
        ...
        private void messageGrid_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                var message = (MessageViewModel) messageGrid.Rows[e.RowIndex].DataBoundItem;

                if (!message.HasBeenViewed) e.CellStyle.Font = new Font(e.CellStyle.Font, FontStyle.Bold);
            }
        }

        private void MainForm_SizeChanged(object sender, EventArgs e)
        {
            if (WindowState == FormWindowState.Minimized && Settings.Default.MinimizeToSysTray) Visible = false;
        }

        private void inspectButton_Click(object sender, EventArgs e)
        {
            foreach (var message in SelectedMessages) InspectMessage(message);
        }
        ...
        private void InspectMessage(MessageViewModel message)
        {
            ...
            var form = new InspectorWindow(message.Parts);
            form.Show();
        ...
        {
            foreach (var session in SelectedSessions) session.ViewLog();
        }
        ...
        private void deleteSessionButton_Click(object sender, EventArgs e)
        {
            foreach (var session in SelectedSessions)
            {
                ...
                foreach (var message in _messages.Where(mvm => session.Session.Messages.Any(m => mvm.Message == m))
                    .ToArray()) _messages.Remove(message);
            }
        }
